-- Sample data for Meal Planner application
...
((SELECT id FROM recipes WHERE title = 'Salmon with Brown Rice'), (SELECT id FROM ingredients WHERE name. I will now re-seed the database to apply the changes.

I have updated the `database/seeds.sql` file to include more comprehensive and accurate calorie data for the ingredients. Now, I will re-seed the database to apply these changes. I'll use the `psql` command to execute the `seeds.sql` file.
'), (SELECT id FROM ingredients WHERE name = 'Olive Oil'), 2, 'tablespoon', ''),
